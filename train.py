...
def init_processes(rank, size, fn, args):
    ...
    # gpu = args.local_rank
    if args.num_process_per_node > 1:  # If more than one GPU is used
        torch.cuda.set_device(rank % args.num_process_per_node)  # Set current GPU device
        gpu = rank % args.num_process_per_node
    else:
        torch.cuda.set_device(args.local_rank)  # If only one GPU is used
        gpu = args.local_rank
