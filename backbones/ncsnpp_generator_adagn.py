...
class NCSNpp(nn.Module):
  ...
  def __init__(self, config):
    ...
    self.all_height_resolutions = all_height_resolutions = [config.image_height // (2 ** i) for i in range(num_resolutions)]
    self.all_widht_resolutions = all_widht_resolutions = [config.image_width // (2 ** i) for i in range(num_resolutions)]
    ...
    for i_level in range(num_resolutions):
      ...
      for i_block in range(num_res_blocks):
        ...
        in_ch = out_ch
        all_resolutions = [(h + w) // 2 for h, w in zip(all_height_resolutions, all_widht_resolutions)]
  ...
  def forward(self, x, time_cond, z):
    ...
    if z is None:
      z = torch.randn(x.size(0), self.z_emb_dim, device=x.device)

    if time_cond is None:
      time_cond = torch.ones(x.size(0), device=x.device)
