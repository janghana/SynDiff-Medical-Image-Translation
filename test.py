...
def sample_and_test(args):
    ...
    #loading dataset
    phase='test'
    dataset=CreateDatasetSynthesis(phase, args.input_path, args.contrast1, args.contrast2)
    ...
    for iteration, (x , y) in enumerate(data_loader): 
        ...
        syn_im1[:,:,iteration]=np.pad(np.squeeze(fake_sample1.cpu().numpy()), ((0, 0), (0, 0)))
        
        loss1[0, iteration] = psnr(fake_sample1, real_data).cpu().numpy()
        ssim_loss1[0, iteration] = compute_ssim(fake_sample1, real_data).cpu().numpy()
    ...
    for iteration, (x , y) in enumerate(data_loader): 
        ...
        real_data = crop(real_data)
        source_data = crop(source_data)
        syn_im2[:,:,iteration]=np.pad(np.squeeze(fake_sample2.cpu().numpy()), ((0, 0), (0, 0)))
        
        loss2[0, iteration] = psnr(fake_sample2, real_data).cpu().numpy()
        ssim_loss2[0, iteration] = compute_ssim(fake_sample2, real_data).cpu().numpy()
    ...
    print('ct to mrt1',np.nanmean(ssim_loss1))
    np.save('{}/ssim_values1.npy'.format(save_dir), ssim_loss1)

    print('mrt1 ti ct', np.nanmean(ssim_loss2))
    np.save('{}/ssim_values2.npy'.format(save_dir), ssim_loss2)
